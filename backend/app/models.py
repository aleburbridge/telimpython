...
class Player(db.Model):
    ...
    role = db.Column(db.String(25), default='')
    prompt_ids = db.relationship('PromptIds', secondary='player_prompt_association')

class Room(db.Model):
    room_code = db.Column(db.String(4), primary_key=True, unique=True)
    story_type = db.Column(db.String(50))
    story = db.Column(db.String(500), default='')
    players = db.relationship('Player', backref='room', lazy=True) #  backref='room' allows you to navigate from a Player back to its Room
